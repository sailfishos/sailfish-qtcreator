...
If the current HEAD, index or working tree differs from the tree denoted by the (latest or selected) tag, a suffix composed from the current branch name, time stamp and a commit hash will be added to the package version. If git-state is not clean a git-stash will be created and its hash will be used instead.  Similarly, hashes will be recorded for all modified git submodules, if any. The formatting of the suffix can be controlled with the 'dirty-version-suffix' configuration option.
